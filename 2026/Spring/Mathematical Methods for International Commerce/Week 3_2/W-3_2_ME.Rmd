...
Unless stated otherwise, assume the base is positive ($a>0$).
...
- The general form of an **exponential expression**:
  ...
  where:
  - **a** is the base - the number being multiplied. Unless stated otherwise, assume the base is positive.
...
$$ \log_a(x) = y \quad \text{if and only if} \quad a^y = x $$ here $a > 0, a ≠ 1, and x > 0.$
...
5. **Use Logs:** Solve $e^{2x} = 10$, hint: take natural logs. $$\ln(10) \approx 2.3026; \text{so } x = \frac{\ln(10)}{2} \approx 1.1513$$ 
...
- **Exponential Functions** describe **growth and decay** in economics, finance, and science.
  ...
  - A common exponential form used in economics is:
    $$ y = a e^{bx} $$
    where:
...
.tiny[In these slides, *ln* denotes the natural logarithm, while *log* means logarithm with a specified base.]
...
where $b$ represents the price elasticity, for the constant-elasticity demand function $Q = aP^b$.
...
getwd()
setwd("C:\\Users\\vyshn\\OneDrive - kdis.ac.kr\\Documents\\GitHub\\Sogang\\2026\\Spring\\Mathematical Methods for International Commerce\\Week 3_2")
