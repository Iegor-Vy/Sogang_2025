...
.tiny[In the market equilibrium model, equilibrium price and equilibrium quantity are endogenous because they are determined by the interaction of demand and supply.
Parameters and outside shocks, such as taxes, subsidies, income, or weather, are exogenous.]
...
where:
- $a$ is the **intercept** (the quantity demanded when price equals zero).
...
# Plot
ggplot(data_demand, aes(x=Qd, y=P)) +
  geom_line(color="blue", size=1.2) +
  labs(title="Linear Demand Function", x="Quantity Demanded (Qd)", y="Price (P)") +
  theme_minimal()
```
...
<div>
.center[<img src="output.png" height="60%" width="60%">]
...
where:
- $c$ is the **intercept** (the quantity supplied when price equals zero).
...
<div>
.center[<img src="output2.png" height="60%" width="60%">]
...
# Plot
ggplot(data_supply, aes(x=Qs, y=P)) +
  geom_line(color="red", size=1.2) +
  labs(title="Linear Supply Function", x="Quantity Supplied (Qs)", y="Price (P)") +
  theme_minimal()
```
...
# Create the plot
ggplot() +
  geom_line(data=data_demand, aes(x=Qd, y=P), color="blue", size=1.2, linetype="solid", label="Demand Curve") +
  geom_line(data=data_supply, aes(x=Qs, y=P), color="red", size=1.2, linetype="solid", label="Supply Curve") +
  geom_point(data=equilibrium, aes(x=Q, y=P), color="black", size=4) +
  labs(title="Market Equilibrium", x="Quantity (Q)", y="Price (P)") +
  theme_minimal()
```
...
- **Equilibrium occurs when** demand equals supply **for all goods** (i.e., when demand equals supply in each market simultaneously).
...
<div>
.center[<img src="output4.png" height="80%" width="80%">]
...
# **Alternative derivation from consumption:**

Since income is either consumed or saved: $Y = C + S$
...
Therefore: $S_0 = -C_0$ and $s = (1-c) = \text{MPS}$
...
In this course, we use Y to denote equilibrium output or income in this simple macro model.
...
3. Work on your Home Assignment #1 (due next Friday, March 19, 12:00).
...
getwd()
setwd("C:\\Users\\vyshn\\OneDrive - kdis.ac.kr\\Documents\\GitHub\\Sogang\\2026\\Spring\\Mathematical Methods for International Commerce\\Week 2_2")
