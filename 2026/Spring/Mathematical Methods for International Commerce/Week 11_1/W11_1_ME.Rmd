...
- Solve constrained optimization problems using substitution.

- Interpret budget, production, and resource constraints.

- Maximize utility subject to a budget constraint.

- Minimize cost subject to a production requirement.

- Verify optimality using second-order conditions.

- Interpret marginal conditions economically.
...
-  Previously, most of the functions we examined were unconstrained, meaning they either had no boundaries, or the boundaries were soft (i.e., variables were not restricted by explicit constraints).
...
- For linear functions, the optimum values typically occur at the boundaries of the feasible region, as the objective function increases or decreases along the edges (typically occur at corner points (vertices) of the feasible region).
...
# Example: Output Maximization under Budget Constraint
...
> After substituting the constraint, the problem becomes a single-variable optimization problem. Therefore, we can use the ordinary second derivative test (No need for Lagrange multipliers and Hessian).
...
$$
x_2 = \frac{400 - 2x_1}{10} = 40 - 0.2x_1
$$
...
- Marginal Utility per Dollar:
  - $\frac{MU_{x_1}}{p_1} = \frac{20}{2} = 10$
  - $\frac{MU_{x_2}}{p_2} = \frac{100}{10} = 10$
...
- Marginal Product per Dollar:
  $$ \frac{MPK}{P_K} = \frac{K^{-1/2}L^{1/2}}{4} $$
  $$ \frac{MPL}{P_L} = \frac{K^{1/2}L^{-1/2}}{3} $$
- At the optimal point, these ratios should be equal, confirming that the firm is minimizing cost effectively.
...
## Instructions:
- **6 groups** with **4-5 students each**.
...
# Group 5: Utility Maximization under Budget Constraint
...
Prices of goods are $p_x = 2$ and $p_y = 4$. The budget is $100$.

- Find the optimal consumption bundle $(x, y)$.
- Verify that marginal utility per dollar is equalized at the optimum.
- Interpret the result economically.

---

# Group 6: Output Maximization under Labor Constraint
...
- Maximize output.
- Find the optimal input levels $L$ and $K$.
...
- (May 14) Lagrange Multipliers (5.6)
...
getwd()
setwd("C:\\Users\\vyshn\\OneDrive - kdis.ac.kr\\Documents\\GitHub\\Sogang\\2026\\Spring\\Mathematical Methods for International Commerce\\Week 11_1")
