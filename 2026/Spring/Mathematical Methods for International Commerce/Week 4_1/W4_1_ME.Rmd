...
# **Percentages and Logarithms**

## **Why combine them?**

- Percentage change measures **relative growth**:
  $$
  \frac{\text{New} - \text{Old}}{\text{Old}}
  $$

- Logarithms measure **proportional change**:
  $$
  \ln(\text{New}) - \ln(\text{Old})
  $$

- Key idea:
  - Logs turn **multiplicative changes → additive changes**
  - Widely used in **economics, finance, and data analysis**

---

# **Log Change ≈ Percentage Change**

## **Core approximation**

$$
\ln(1 + x) \approx x \quad \text{(for small } x\text{)}
$$

So:

$$
\ln(\text{New}) - \ln(\text{Old}) \approx \text{percentage change}
$$
...
- Log change:
  $$
  \ln(110) - \ln(100) \approx 0.095 \approx 9.5\%
  $$
...
# **Why Economists Use Log Changes**

## **Advantages**

- Handles **large numbers easily**

- Converts growth into **simple differences**
...
1. Price rises from 200 to 220  
   - Exact % change = ?  
   - Log change ≈ ?

2. GDP increases from 1000 to 1050  
...
A 1% increase on a 5% rate means:
$$
5\% \times 1\% = 0.05\% \text{ increase}
$$

so the rate rises from 5.00% to 5.05%.
...
$$
if the price index is based at 100
...
- **Interpretation**:  
  Spending in **Y3** is 3.8% **above** the Y2 level.
...
We are solving for the **growth rate** denoted as **r**.

This assumes the same percentage growth rate each year.
...
- **EAR**  is the effective annual rate that accounts for compounding.
...
### **Compound Interest**
5. Calculate the compound amount on $2000 at 6% for 5 years.
6. Using the same principal, interest rate, and time period, compare the amount under annual and continuous compounding.
...
- (Mar 26) Geometric Series (3.3), Investment Appraisal (3.4)  
...
getwd()
setwd("C:\\Users\\vyshn\\OneDrive - kdis.ac.kr\\Documents\\GitHub\\Sogang\\2026\\Spring\\Mathematical Methods for International Commerce\\Week 4_1")
