...
- Differentiate the outer function first (keeping inner unchanged), then multiply by derivative of the inner function.
...
- Derivative:
$$
\frac{dy}{dx} = \frac{1/x \cdot x^2 - \ln(x) \cdot 2x}{x^4} = \frac{x - 2x\ln(x)}{x^4} = \frac{1 - 2\ln(x)}{x^3}
$$
...
$$ R(t) = 5000e^{0.05t} $$
This means revenue grows at **5% of its current level** per time unit.
...
---
# Application: Continuous Revenue Growth (cont'd)

Interpretation:
- The **proportional growth rate** is constant: $$\frac{R'(t)}{R(t)} = \frac{250e^{0.05t}}{5000e^{0.05t}} = 0.05$$.
- The **dollar increase is not constant** because $$R'(t)=250e^{0.05t}$$ rises over time.
- Common in modeling investment returns, inflation, or GDP.
---
# Application: Continuous Revenue Growth (cont'd)

Quick check:
- At $t=0$, $R(0)=5000$ and $R'(0)=250$ (5% of 5000).
- At $t=10$, $R(10)=5000e^{0.5} \approx 8243.6$ and $R'(10)=0.05\,R(10) \approx 412.2$.

So, the **percentage rate (5%) is constant**, but the **absolute increase** gets larger as revenue grows.


---

# Application: Growth Rate in Exponential Model
...
# Application: Growth Rate in Exponential Model (cont'd)

### Find:
- $$ \frac{dQ}{dt} = rAe^{rt} = rQ(t) $$

The term $\frac{1}{Q}\frac{dQ}{dt}=r$ represents the constant proportional (continuous) growth rate (semi-elasticity) of $Q$. This means that at every point in time, the instantaneous rate of change of $Q$ is a constant fraction of its current level.
...
# Example: Growth Rate in Exponential Model
...
### Step 2: Proportional Growth Rate
...
- Proportional growth rate (semi-elasticity) is **constant**: $E = 0.03$ 
...
- (April 21) Mid term exam (in class):
...
- (April 28) Functions of Several Variables (5.1) 
...
getwd()
setwd("C:\\Users\\vyshn\\OneDrive - kdis.ac.kr\\Documents\\GitHub\\Sogang\\2026\\Spring\\Mathematical Methods for International Commerce\\Week 7_2")
