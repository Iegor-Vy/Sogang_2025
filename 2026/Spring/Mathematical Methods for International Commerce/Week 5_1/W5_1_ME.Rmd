...
- The slope is **not constant** for non-linear functions, but we approximate it using the derivative (tangent slope).
...
Profit maximization occurs where $\Pi'(x) = 0$ and $\Pi''(x) < 0$.
...
- Since $\Pi''(x) < 0$, the profit function is **concave down**, this confirms the critical point is a maximum.
...
- A4 double-sided cheat sheet with formulas allowed (handwritten).
...
getwd()
setwd("C:\\Users\\vyshn\\OneDrive - kdis.ac.kr\\Documents\\GitHub\\Sogang\\2026\\Spring\\Mathematical Methods for International Commerce\\Week 5_1")
