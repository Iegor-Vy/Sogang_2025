...
subtitle: "Week 13: Special Lecture on Modern Analytics Workflow in Business and Research // How Real Data Projects Are Built Today"
...
ROLE: Answering as an expert in statistics education, data science workflows, and computational research infrastructure.

TL;DR: Below is a complete xaringan lecture deck for “Modern Analytics Workflow in Business and Research,” designed specifically for your undergraduate International Commerce students. It is practical, visual, inspirational, and intentionally non-technical.

ANSWER:

```r id="j4m7wp"
---
title: "Modern Analytics Workflow in Business and Research"
subtitle: "How Real Data Projects Are Built Today"
author: "Iegor Vyshnevskyi, Ph.D."
output:
  xaringan::moon_reader:
    css: [default, metropolis, metropolis-fonts]
...
```{r setup, include=FALSE} id="u6r9hf"
...
* reports in Overleaf or PowerPoint
* AI tools
...
| Coding            | Python, R         |
| Development       | VS Code           |
| Cloud notebooks   | Google Colab      |
| Version control   | GitHub            |
| Writing/reporting | Overleaf          |
...
.center[ <img src="https://code.visualstudio.com/assets/home/home-screenshot-mac-2x-v2.png" width="75%">
]
...
Overleaf is a collaborative writing platform.
...
| Career            | Uses                     |
| ----------------- | ------------------------ |
| Data analyst      | dashboards, reports      |
| Business analyst  | strategy decisions       |
| Research analyst  | statistical analysis     |
| Financial analyst | forecasting              |
| Consultant        | client analytics         |
| Policy analyst    | evidence-based decisions |
...
SOURCES:
- [1] User course profile and workflow context from this chat
- [2] Standard modern analytics workflow practices in business and research
- [3] Public documentation for VS Code, GitHub, Overleaf, and Google Colab
...
# ANOVA Intuition: What Are We Actually Measuring?

Suppose a company compares customer spending across 3 regions:
...
$$
\bar{x}_{overall} = 72.5
$$

ANOVA asks:

> "Why are observations different from each other?"

There are only two possible reasons:

1. **Natural randomness inside each group** - customers inside the same region are not identical

2. **Actual differences between group averages** - regions themselves may behave differently

ANOVA separates total variation into these two parts.

---

# The Three Important Quantities

## 1. SST = Total Variation

Measures:

> "How far are ALL observations from the overall average?"

It ignores groups completely.

Large SST:
- observations overall are very spread out

---

## 2. SSE = Within-Group Variation (Error, SSW)

Measures:

> "How far are observations from THEIR OWN group average?"
...
If customers inside each region behave similarly:
- SSE becomes smaller

---

## 3. SSTr = Between-Group Variation (Treatment, SSB)

Measures:

> "How far are group averages from the overall average?"

This captures:
- actual group differences

If regional averages differ strongly:
- SSTr becomes larger
...
ANOVA always splits total variation:
...
| Region | Values | Group Mean |
|---|---|---|
| Asia | 70, 75 | 72.5 |
| Europe | 60, 65 | 62.5 |
| North America | 80, 85 | 82.5 |
...
# Step 2. Compute SSE (Within-Group Variation)

SSE asks:

> "How different are customers INSIDE the same group?"
...
Customers inside each region differ only a little.

---
class: my-one-page-font-table
# Step 3. Compute SSTr (Between-Group Variation)

SSTr asks:

> "How different are GROUP AVERAGES from the OVERALL average?"
...
Regional averages are VERY different.

.tiny[We multiply by 2 because there are 2 observations in each group, so the group mean's deviation from the overall mean is counted for each observation in that group.]
...
## To check: calculate SST directly:
$$
(70-72.5)^2 + (75-72.5)^2 + (60-72.5)^2 + (65-72.5)^2 + (80-72.5)^2 + (85-72.5)^2 = 437.5
$$
...
# Plain English Meaning of F
.pull-left[
## Small F

- groups are similar
- within-group randomness dominates

Meaning:
> differences probably happened by chance
]
.pull-right[
## Large F

- group averages differ a lot
- within-group randomness is relatively small

Meaning:
> real group differences likely exist
]
---
...
- customers INSIDE each region are relatively similar
- regional averages are very different
...
- spending truly differs across regions
...
> "Are group differences large relative to normal randomness inside groups?"
